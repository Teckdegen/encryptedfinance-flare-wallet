...
async function main(){
  ...
  const base=await nsc(); console.log("note",ethers.formatUnits(n.amount,6),"| NoteSpent base",base);
  const r=await c.privateTransfer(n,"0x000000000000000000000000000000000000dEaD",ethers.parseUnits("3",6)); console.log("TRANSFER jobId",r.jobId||r.txHash);
  for(let i=0;i<12;i++){await new Promise(r=>setTimeout(r,12000));const c2=await nsc();console.log(`t+${(i+1)*12}s NoteSpent=${c2}`);if(c2>base){console.log("TRANSFER SETTLED ✅ (delivery still works)");return;}}
  console.log("TRANSFER NOT settled ❌ (delivery decayed for everything)");
}
